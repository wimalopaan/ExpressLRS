...
void SerialRelay::forwardMessage(const crsf_header_t *message)
{
...
void SerialRelay::sendRCFrame_part(uint32_t *channelData, const bool isHigh)
{
...
uint32_t SerialRelay::sendRCFrame(bool frameAvailable, bool frameMissed, uint32_t *channelData) {
    if (!frameAvailable)
...
void SerialRelay::processBytes(uint8_t *bytes, const uint16_t size)
{
